...
def bandwidth_cutoff(width, freqList, centre=400):
    """width: detector bandwidth in nanometres, centred at centre, 400nm by default. Returns indices"""
...
def barc_lin_centre_mean(D, params):
    """params = A, w0, sigmat"""
    ...
    dw = np.sqrt(np.log(4)/np.pi) * eps / (A * sigmat)

    wcent = 2*w0 - dw

    return 2 * np.pi * c / wcent * 1e-6

def barc_lin_centre_median(D, params):
    """params = A, w0, sigmat"""
    ...
    dw = np.sqrt(np.log(4)) * erfinv(0.5) * eps / (A * sigmat)
...
def barc_lin_centre_paper(D, params):
    ...
    sigmaw = np.sqrt(2) * np.sqrt(np.log(4)) / sigmat

    print(f"bandwidth? {sigmaw}")
        ...
        for k, L in enumerate(Lvals):
            ...
            for m, D in enumerate(Dvals):
                ...
                if L in hotLvals and D in hotDvals:
                    ...
                    plt.axhline(y=wavelength_position(cent,freqList,lowplot,highplot), color='black', linewidth=1.2, linestyle='-')

                    if det_bandwidth < plot_bandwidth:
                        print(f"line? {cent+det_bandwidth/2}")
                        print(wavelength_position(400 + det_bandwidth/2,freqList,lowplot,highplot))
                        print(wavelength_position(cent + det_bandwidth/2,freqList,lowplot,highplot))
                        plt.axhline(y=wavelength_position(cent + det_bandwidth/2,freqList,lowplot,highplot), color='black', linewidth=0.8, linestyle='--')
                        plt.axhline(y=wavelength_position(cent - det_bandwidth/2,freqList,lowplot,highplot), color='black', linewidth=0.8, linestyle='--')
